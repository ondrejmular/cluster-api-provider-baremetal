...
func TestIsActionTimedOut(t *testing.T) {
	machine, _ := getMachine("machine1")
	annotation := "annotationName"
	// no annotations
	if isActionTimedOut(machine, annotation, 1*time.Second) {
	...
	machine.Annotations[annotation] = "not a timestamp"
	if isActionTimedOut(machine, annotation, 1*time.Second) {
		t.Errorf("Expected 'false' when annotation is not a valid timestamp")
	}
	machine.Annotations[annotation] = time.Now().Add(-1 * time.Second).Format(time.RFC3339)
	if isActionTimedOut(machine, annotation, 1*time.Hour) {
		t.Errorf("Expected 'false' when timeout has not passed yet")
	}
	machine.Annotations[annotation] = time.Now().Add(-1 * time.Hour).Format(time.RFC3339)
	if !isActionTimedOut(machine, annotation, 1*time.Second) {
